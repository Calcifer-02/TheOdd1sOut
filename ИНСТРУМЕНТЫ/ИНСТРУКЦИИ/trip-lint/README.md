...
TypeScript по мировой практике (ADR-0005). Волна 4 — сделано: осмотр
конфигурации `trip-lint doctor` (молчаливое отключение пресета), сравнение
полных снимков (`--previous`), миграции EF Core по сценарию (ADR-0006);
интеграция: `trip verify --lint`, перехват
