...
for (const [идентификатор, пин] of Object.entries(СРЕДСТВА_ШАБЛОНА)) {
  ...
  обновление(
    ...
    () => {
      ...
      // Платформы берутся из самой записи: ходунки могли объявлять их меньше,
      // чем публикует выпуск, и дописывать неизвестную платформу скрипт не
      // вправе.
      for (const платформа of Object.keys(это.запись.sha256 ?? {})) {
