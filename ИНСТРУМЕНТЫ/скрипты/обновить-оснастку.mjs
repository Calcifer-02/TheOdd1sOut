...
// Коннектор трекера — один архив для всех платформ: чистый Python, ключ «any».
// Отпечаток берётся из опубликованного файла выпуска, а не из локальной
// сборки: архив упаковывает время файлов, и сборка на другой машине даёт
// другой отпечаток при том же содержимом.
const TRIP_SHIVA = {
  версия: '0.2.0',
  отпечатки: {
...
обновление(
  `ходунки средств: trip-shiva-connector ${TRIP_SHIVA.версия}`,
  () => {
    const файл = 'ИНСТРУМЕНТЫ/training-wheels.json';
    if (!есть(файл)) return 'в проекте нет ходунков средств';
    const ходунки = JSON.parse(читать(файл));
    const средства = Array.isArray(ходунки) ? ходунки : ходунки.tools;
    const запись = средства?.find((с) => с.id === 'trip-shiva-connector');
    if (!запись) return 'в ходунках нет записи trip-shiva-connector';
    if (запись.version === TRIP_SHIVA.версия) return 'применено';
    if (сравнитьВерсии(запись.version, TRIP_SHIVA.версия) > 0) {
      return `в ходунках версия ${запись.version} новее ${TRIP_SHIVA.версия} — понижать не буду`;
    }
    return 'нужно';
  },
  () => {
    ...
    const средства = Array.isArray(ходунки) ? ходунки : ходунки.tools;
    const запись = средства.find((с) => с.id === 'trip-shiva-connector');
    запись.version = TRIP_SHIVA.версия;
    запись.sha256 = { ...запись.sha256, any: TRIP_SHIVA.отпечатки.any };
    писать(файл, `${JSON.stringify(ходунки, null, 2)}\n`);
    // Инструкции средства — копии, а не канон, и едут в самом файле выпуска.
    // Скрипт их не переносит: тела страниц живут в выпуске, а не в шаблоне, и
    // вторая копия в скрипте разошлась бы с ними на первой же правке.
    внимание.push(
      'trip-shiva-connector: после установки обновите копии в ' +
        'ИНСТРУМЕНТЫ/ИНСТРУКЦИИ/trip-shiva-connector/ из .trip/tools — ' +
        'в выпуске 0.2.0 появились ПОДКЛЮЧЕНИЕ.md и CHANGELOG.md',
    );
  },
);
