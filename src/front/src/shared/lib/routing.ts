...
/**
 * Представление экрана — телефон или рабочее место — закрепляется параметром
 * адреса, а не отдельным путём.
 *
 * Путь `/mobile` пришлось бы завести рядом с каждым экраном сервиса
 * (`/cabinet` и `/mobile/cabinet`, `/landfills` и `/mobile/landfills`):
 * маршруты удвоились бы, а уже разосланные ссылки, несущие выборку в строке
 * поиска (`#/?calc=…&sort=…`), перестали бы открывать закреплённое
 * представление. Параметр к пути безразличен и к соседним параметрам тоже:
 * он закрепляет представление на любом экране, ничего не удваивая
 * (R-085, AC-085c; дизайн-договор, разд. 4.5).
 *
 * Здесь объявлено только имя параметра: какие значения допустимы, знает
 * представление (`./viewport`). Обратная зависимость замкнула бы общий слой
 * в кольцо — адрес о представлении не знает ничего.
 */
...
/**
 * Копия состояния адреса с закреплённым представлением. Именно копия:
 * разобранный маршрут кешируется по строке адреса, и правка его на месте не
 * дошла бы ни до адресной строки, ни до подписчиков маршрута.
 */
export function withViewParam(query: URLSearchParams, view: string): URLSearchParams {
