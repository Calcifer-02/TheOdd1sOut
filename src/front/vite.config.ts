...
export default defineConfig({
  ...
  test: {
    ...
    // Предел ожидания поднят с пяти секунд: набор идёт сорока шестью файлами
    // одновременно, и проверки окна маршрута — самые тяжёлые в наборе: карта,
    // портал, служба маршрутизации. По одиночке они укладываются в две секунды,
    // а под общей нагрузкой не успевали — и это был отказ машины, а не кода.
    testTimeout: 15000,
  },
});
