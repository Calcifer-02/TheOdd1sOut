...
//
// Заглушка хранит справочник и меняет его так же, как служба: удачная
// правка тарифа двигает дату актуальности цен (R-048), подтверждённый
// импорт применяет расхождения и записывает прогон обновления (AC-044d).
// Без этого проверка измеряемой метрики пути 3 была бы проверкой заглушки,
// а не интерфейса.
//
...
export function installMaintenanceStub(): MaintenanceStub {
  ...
  function startImport(): StubResponse {
    return {
      ...
      body: { id: IMPORT_ID, kind: 'tariffs', changes: copy(IMPORT_CHANGES), rejectedRows: [] },
    };
  }
