 ...
 * флажок строки номером вместо полигона, снимите признак выравнивания с
 * ячейки шапки, уберите подпись таблицы или показ пустого результата — они
 * упадут.
 *
...
describe('таблица сравнения', () => {
  ...
  it('числовой столбец объявлен выровненным по правому краю и в шапке, и в ячейке', () => {
    render(таблица({ rows: [ВОСТОК] }));

    const строка = within(screen.getAllByRole('rowgroup')[1]).getAllByRole('row')[0];
    const ячейки = within(строка).getAllByRole('cell');

    // Столбец назван значением, а не номером: перестановка столбцов не должна
    // тихо перевести проверку на соседний.
    const итого = ячейки.find(ячейка => ячейка.textContent === String(ВОСТОК.total));

    expect(screen.getByRole('columnheader', { name: /Итого/u })).toHaveAttribute('data-align', 'end');
    expect(итого, 'заголовок и значение одного столбца выровнены одинаково').toHaveAttribute('data-align', 'end');
    expect(
      screen.getByRole('columnheader', { name: 'Статус' }),
      'нечисловой столбец остаётся у левого края',
