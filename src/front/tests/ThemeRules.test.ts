...
function paddingLeft(body: string): number {
  ...
  const sides = (found?.[1] ?? '').trim().split(/\s+/).map((side) => Number.parseInt(side, 10));
...
describe('таблица сравнения (BUG-001, BUG-003)', () => {
  it('ячейка с многострочным содержимым растит строку, а не режет её', () => {
    ...
    expect(body, 'высота строки сравнения — наименьшая, а не мера').toContain(
      `min-height: ${layout.rowHeight}px`,
    );
    expect(body, 'жёсткая высота обрезала бы название, адрес и перечень тарифов').not.toMatch(
      /(^|[^-])height:\s*\d/,
    );
  });
...
describe('ссылка общего слоя (BUG-002)', () => {
  ...
  it('показывает фокус кольцом, а не одной сменой цвета', () => {
    expect(ruleBody('.imolt-link:focus-visible')).toContain(
