...
/// проверка предела удаления и повторов отправки (R-081, R-082) обязана
/// двигать время сама, иначе она ждёт по-настоящему и зависит от скорости
/// машины (правила проекта, «Детерминизм там, где важен повтор»).
///
/// Маркер здесь не ставится: тип-делегат применимой единицей не считается, и
/// связь всё равно не создалась бы — её несут MaxDelivery и BotOptions.
