-- Начальный набор данных ИМОЛТ: демонстрационный, не справочный.
--
-- Источник большинства значений — канонический пример договора
-- (src/back/Imolt.Api/contracts/openapi.yaml, набор ConcreteCalculation):
-- адрес «г Москва, ул Годовикова, д 9», группы beton-lom и kirpich-lom,
-- полигоны vostok-timohovo и iksha, дата актуальности 17.09.2026.
-- Арифметика примера сходится с формулой R-018: 20 т × 12,00 ₽/т-км × 45 км
-- = 10 800,00 ₽ перевозки и 20 т × 450,00 ₽/т = 9 000,00 ₽ утилизации.
--
-- Значения, которых в договоре нет, помечены словом «демонстрационное» прямо
-- в строке ниже. Их подтверждает заказчик: регламент актуализации цен —
-- открытый вопрос Q-001, редакция каталога ФККО — Q-015. Выдавать их за
-- справочные данные нельзя (правила проекта: величины расчёта — предмет
-- требований и справочников, а не выбора на месте).
--
-- Набор применяется поверх начальной схемы и повторяется без вреда:
-- on conflict do nothing оставляет уже изменённые записи как есть, иначе
-- повторный запуск затирал бы правку менеджера данных.
