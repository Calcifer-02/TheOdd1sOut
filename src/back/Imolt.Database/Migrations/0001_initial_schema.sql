...
--
-- Источник состава — инженерная модель проекта (docs/АРТЕФАКТЫ_ПРОДУКТА_И_БИЗНЕСА/
-- ИНЖЕНЕРНАЯ_МОДЕЛЬ.md, сущности СУЩ-01 — СУЩ-15). Схема — общий контракт
-- расчётной части и будущей службы сбора (КНТ-02, ADR-0002), поэтому она
-- читается глазами и меняется двумя единицами совместно.
--
-- Денежные величины — numeric(12,2): двоичная дробь округляет рубли по
-- дороге, а смета этого не прощает.
-- Перечисления заданы ограничением CHECK, а не типом ENUM: добавление
-- значения в ENUM нельзя откатить внутри транзакции, и миграция перестаёт
-- быть отменяемой.
